...
pnpm exec wrangler d1 execute DB --local --file=work/demo.sql
pnpm dev
```
...
2. 建立 Turnstile widget，允許 `comments.rurichan.work` 及博客預覽域名。
...
公開端點為 `GET /api/v1/comments?article=...`、`POST /api/v1/comments` 及 `GET /api/v1/config`。管理端點需要 GitHub OAuth session。文章識別碼建議使用 `path:/year/month/day/title/`，Hexo front matter 可用 `comment_id` 固定識別碼。
